...
data "external" "cam_tenant" {
  #depends_on = ["data.external.cam_token"]
...
resource "null_resource" "create_instance" {
  ...
  provisioner "local-exec" {
    ...
    command = "/bin/bash ./run_import.sh --cam_host_name=${var.cam_hostname} --cam_token=${data.external.cam_token.result["token"]} --cam_tenant=${data.external.cam_tenant.result["tenant"]} --cam_instance_name=${var.cam_instancename} --cam_instance_namespace=${var.cam_namespace} --cam_cloudconnection_name=${var.cam_cloudconnection_name} --cam_template_name=\"${var.cam_template_name}\" --cam_template_version_name=${var.cam_template_version_name} --cloud_instance_id=${var.cloud_instance_id}" 
  }
}
